...
export class CitiesController {
  ...
  findAllWithFilter(@Req() req: Request, @Res() res: Response) {
    ...
    const citiList = this.cityService.getListCity(filterObject);
    citiList.then((cities) => {
  ...
  @Post()
  @Roles([ROLE_LIST.ADMIN])
  createNewCity(@Req() req: Request, @Res() res: Response) {
    const data = req.body;
    const response = this.cityService.createCity(data);
